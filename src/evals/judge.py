...
def assess_quality(task: str, response: object, acceptance_criteria: str, client: LLMClient | None = None) -> tuple[JudgeResult, str]:
    ...
    return JudgeResult(passed=useful, quality_score=0.8 if useful else 0.0, rationale="Deterministic fallback: response contains the required explanatory narrative."), "heuristic_fallback"
